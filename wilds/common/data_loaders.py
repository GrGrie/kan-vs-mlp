...
def get_ssl_train_loader(loader, dataset, batch_size,
        uniform_over_groups=None, grouper=None, **loader_kwargs):
    """
    ...
    """
    assert loader == 'standard'  # we do not have group information while ssl training
    if uniform_over_groups is None or not uniform_over_groups:
        ...
        sampler = WeightedRandomSampler(weights, len(dataset), replacement=True) # TODO - replace this with sometihng compatilble with distributed training
        return DataLoader(
            dataset,
...
def get_train_loader(loader, dataset, batch_size,
        uniform_over_groups=None, grouper=None, distinct_groups=True, n_groups_per_batch=None, sampling=None, minority_weight=None, **loader_kwargs):
    """
    Constructs and returns the data loader for training.
    Args:
        - loader (str): Loader type. 'standard' for standard loaders and 'group' for group loaders,
    ...
    """
    if loader == 'standard':
        if sampling is None and minority_weight is None and (uniform_over_groups is None or not uniform_over_groups):
            return DataLoader(
                dataset,
                shuffle=True, # Shuffle training dataset
                sampler=None,
        ...
        if minority_weight is not None:
            groups, group_counts = grouper.metadata_to_group(
                dataset.metadata_array,
                return_counts=True)
            og_group_counts = group_counts if dataset.og_group_counts is None else dataset.og_group_counts 
            constant_group_weights = torch.ones_like(group_counts) * (1. - minority_weight)
            constant_group_weights[og_group_counts <= og_group_counts.median()] = minority_weight
            print('constant group counts : ', og_group_counts, og_group_counts.median())
            print('constant group weights: ', constant_group_weights)

            group_weights = constant_group_weights / group_counts
        ...
        elif uniform_over_groups or sampling is not None:
            assert grouper is not None
            groups, group_counts = grouper.metadata_to_group(
                dataset.metadata_array,
                return_counts=True)
            ...
            if sampling == '':
                ...
                sampler = WeightedRandomSampler(weights, int(len(group_counts) * int(max(group_counts).item())), replacement=True)
            elif sampling == 'downsample':
                sampler = WeightedRandomSampler(weights, int(len(group_counts) * int(min(group_counts).item())), replacement=False)
